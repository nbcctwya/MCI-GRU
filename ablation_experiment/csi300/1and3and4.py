...
class StockPredictionModel_1_3_4(nn.Module):
    def __init__(self, input_size, hidden_size, gat_in_channels, gat_out_channels, gat_heads, num_hidden_states):
        super(StockPredictionModel_1_3_4, self).__init__()
        self.attention_gru = AttentionGRUCell(input_size, hidden_size)
        self.cross_attention_1 = CrossAttention(hidden_size)
        self.cross_attention_2 = CrossAttention(gat_in_channels)
        self.market_hidden_states_1 = nn.Parameter(torch.randn(num_hidden_states, hidden_size))  # 市场隐状态
        self.market_hidden_states_2 = nn.Parameter(torch.randn(num_hidden_states, gat_in_channels))
        self.final_gat = GATLayer(hidden_size + gat_in_channels, gat_out_channels, hidden_size + gat_in_channels, gat_out_channels, gat_heads)
    ...
    def forward(self, x_time_series, x_graph, edge_index, edge_weight):
        ...
        # 3. 应用交叉注意力
        stock_rep_1 = self.cross_attention_1(h_gru_last.unsqueeze(1), self.market_hidden_states_1, self.market_hidden_states_1).squeeze(1)  # B1
        stock_rep_2 = self.cross_attention_2(x_graph.unsqueeze(1), self.market_hidden_states_2, self.market_hidden_states_2).squeeze(1)  # B2

        # 4. 拼接处理后的特征
        concatenated_output = torch.cat([h_gru_last, stock_rep_1, stock_rep_2], dim=1)

        # 通过GAT层进行预测
        final_output = self.final_gat(concatenated_output, edge_index, edge_weight)
...
def model_train_predict_1_3_4(num_models, num_epochs, save_path, model_dt, kdcode_last, df3_2_dt, train_time_series_loader, train_graph_loader, X_test_time_series, test_graph_loader):
    device = torch.device('cuda:1' if torch.cuda.is_available() else 'cpu')
    for num in range(num_models):
        model = StockPredictionModel_1_3_4(
            input_size=num_features,
            hidden_size=32,
            gat_in_channels=num_features,
            gat_out_channels=4,
...
# nohup /home/liyuante/miniconda3/envs/py38/bin/python /home/liyuante/nerocomputing24/ablation_experiment/csi300/1and3and4.py >> /home/liyuante/neruocomputing/log_for_all/csi1and3and4.txt 2>&1 &