...
class StockPredictionModel(nn.Module):
    def __init__(self, input_size, hidden_size, hidden_size_gat1, output_gat1, gat_in_channels, gat_out_channels, gat_heads, hidden_size_gat2, embed_dim, num_hidden_states):
        ...
        self.attention_gru = AttentionGRUCell(input_size, hidden_size)  # Attention GRU layer
        self.gat_layer = GATLayer(hidden_size_gat1, output_gat1, gat_in_channels, gat_out_channels, gat_heads)  # First GAT layer
        self.cross_attention = CrossAttention(hidden_size)  # Cross-attention layer
    ...
    def forward(self, x_time_series, x_graph, edge_index, edge_weight):
        ...
        # Process time series data through GRU
        for t in range(num_time_steps):
            h_gru = self.attention_gru(x_time_series[:, :, t, :], h_gru)
        h_gru_1 = h_gru[-1,:,:]
        # print(h_gru_1.shape)

        # Process graph data through GAT layer
        x_gat = self.gat_layer(x_graph, edge_index, edge_weight)
        # print(x_gat.shape)

        # Use the market hidden states R to access the stock representations S
        # torch.Size([285, 32])
        ...
        # Concatenate the outputs of the four parts
        ...
        # Make predictions through the final GAT layer
